%% supplemental material, demo5 MUA
% =========================================================================
%
% Demo of example 4 (Jasper Ridge image) of the paper:
% 
...
% HERE YOU NEED TO LOAD THE JASPER RIDGE IMAGE FILE.
addpath('real_data')
...
Y = Y/maxValue;
nBand = size(Y,1); % get the correct number of bands
...
load('spectral_library_jasperRidge.mat')
n = size(A,2);
...
figure
[ha, pos] = tight_subplot(4,4,[.025 .025],[.05 .05],[.05 .15]);
...
%     imagesc(X_hat_tv_im2(:,:,i), [0 1]), axis square, set(gca,'xtick',[]), set(gca,'xticklabel',[]), set(gca,'ytick',[]), set(gca,'yticklabel',[])
    ...
    axes(ha((i-1)*numAlgs + 2));
    imagesc(X_hat_l21LC_im2(:,:,i), [0 1]), axis square, set(gca,'xtick',[]), set(gca,'xticklabel',[]), set(gca,'ytick',[]), set(gca,'yticklabel',[])
...
axes(ha((1-1)*numAlgs + 1)); ylabel('Tree','interpreter','latex')
axes(ha((2-1)*numAlgs + 1)); ylabel('Water','interpreter','latex')
axes(ha((3-1)*numAlgs + 1)); ylabel('Dirt','interpreter','latex')
axes(ha((4-1)*numAlgs + 1)); ylabel('Road','interpreter','latex')
...
mkdir('examples')
print('examples/estim_abundances_jasperRidge','-dpdf')




figure, imagesc(1.2*Yim(:,:,[110 40 20]))
set(gca,'xtick',[]), set(gca,'xticklabel',[]), set(gca,'ytick',[]), set(gca,'yticklabel',[])
print('examples/jasper_im','-dpdf')
